...
1. 首次启动会进入引导页：打开本地仓库、从 GitHub / Gitee / GitLab 等克隆，或新建本地仓库。若还没有远程仓库，可先到托管平台创建，再把地址克隆回来
2. 之后也可以用工具栏「浏览」或菜单 **文件 → 打开仓库**（Ctrl/Cmd+O）切换仓库
3. 中间是节点树：字符串节点、系列线（子提交）、恒定线（旁支贯穿）、虚父（未载入的父提交）
4. 左侧管理分支、远程、标签、Stash
5. 右侧查看工作区、提交详情、文件树、Blame、Reflog、引用对比
6. 底部状态栏会显示当前使用的是「内置 Git」还是「系统 Git」及其版本
